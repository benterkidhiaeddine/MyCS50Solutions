   ...
   for (int i =0;i<height;i++)
   {
    for (int j=0;j<width;j++)
    {
        ...
        for (int x=0;x<3;x++)
        {
            ...
            for(int y=0;y<3;y++)
            {
                 if(i-1+x<0||i-1+x>height-1 ||j+y-1 <0||j+y-1>width-1)
                 {
                    continue;
                 }
                 redx=redx+image[i-1+x][j-1+y].rgbtRed * Gx_arr[x][y];
                 redy=redy+image[i-1+x][j-1+y].rgbtRed * Gy_arr[x][y];

                 bluex=bluex+image[i-1+x][j-1+y].rgbtBlue * Gx_arr[x][y];
                 bluey=bluey+image[i-1+x][j-1+y].rgbtBlue * Gy_arr[x][y];

                 greenx=greenx+image[i-1+x][j-1+y].rgbtGreen * Gx_arr[x][y];
                 greeny=greeny+image[i-1+x][j-1+y].rgbtGreen * Gy_arr[x][y];
            }
        }
